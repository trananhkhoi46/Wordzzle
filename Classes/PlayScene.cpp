...
int riddlePacketId;
Scene* PlayScene::scene(int riddle_packet_id) {
...
bool PlayScene::init() {
	...
	btnFacebook->addTouchEventListener(
			CC_CALLBACK_2(PlayScene::facebookButtonCallback, this));
...
void PlayScene::initRiddleButtons() {
...
//	scrollview->setBackGroundColor(Color3B(200, 200, 200)); //Background
	...
	CCLog("bambi PlayScene - inner height: %f, frame hight: %f",
...
void PlayScene::facebookButtonCallback(Ref* pSender,
		ui::Widget::TouchEventType eEventType) {
	if (eEventType == ui::Widget::TouchEventType::ENDED) {
